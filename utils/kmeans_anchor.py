...
if __name__ == "__main__":
    ...
    for idx in range(len(dataset)):
        if idx % 5000 == 0:
            print('Loading voc data [%d / %d]' % (idx + 1, len(dataset)))

        # For VOC
        target = dataset.pull_anno(idx)
        ...
        for tgt in target:
            ...
            if args.dataset in ['ucf24', 'jhmdb21']:
                bw = (x2 - x1) / 320 * args.img_size
                bh = (y2 - y1) / 240 * args.img_size
            elif args.dataset == 'aihub_park':
                bw = (x2 - x1) / 1920 * args.img_size
                bh = (y2 - y1) / 1080 * args.img_size
            # check bbox
            if bw < 1.0 or bh < 1.0:
                continue
    ...
    print("Start k-means !")
    centroids = anchor_box_kmeans(
        total_gt_boxes=boxes,
